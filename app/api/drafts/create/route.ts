  ...
  try {
    ...
    if (!ownerId || !title) {
      return NextResponse.json(
        { error: "Missing required fields: ownerId and title are required" },
        { status: 400 },
      );
    }
    ...
    if (collaborators && collaborators.length > 0) {
      ...
      const invalidUserIds = uniqueCollaborators.filter(
      ...
      await Promise.all(
        uniqueCollaborators.map(async (userId: string) => {
          ...
          console.log(
            `[Notification] Preparing to send notifications for user: ${userId}`,
          );
          const deviceTokens = await prisma.deviceToken.findMany({
            where: { userId },
          });
          console.log(
            `[Notification] Found ${deviceTokens.length} device tokens for user ${userId}`,
          );
          ...
          await Promise.all(
            deviceTokens.map(async (dt) => {
              console.log(
                `[Notification] Sending to device: ${dt.platform} (${dt.token.slice(0, 10)}...)`,
              );
              try {
                const message = {
                  notification: {
