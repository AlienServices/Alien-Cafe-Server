...
async function fallbackFileMigration(
  ...
  try {
    ...
    console.log(
      "Fallback migration successful for file:",
  ...
  try {
    ...
    if (!supabaseUrl || !supabaseServiceKey) {
      return NextResponse.json(
        {
          error:
            "Server configuration missing Supabase URL or Service Role Key",
        },
        { status: 500 },
      );
    }
    ...
    const linkPreviewsToUse =
      linkPreviews ||
      draft.linkPreviews.map((lp: any) => ({
        url: lp.url,
        title: lp.title,
    ...
    console.log(
      "🎬 Link previews to use (request or saved):",
      JSON.stringify(
        linkPreviewsToUse.map((p: any) => ({
          url: p.url,
          isVideo: p.isVideo,
    ...
    const isUpdate =
      draft.originalPostId !== null && draft.originalPostId !== undefined;
    ...
    if (isUpdate) {
      ...
      post = await prisma.post.update({
        ...
        data: {
          ...
          linkPreviews:
            linkPreviewsToUse && Array.isArray(linkPreviewsToUse)
              ? {
                  create: linkPreviewsToUse.map((preview: any) => ({
                    ...
                    isVideo:
                      preview.isVideo === true || preview.isVideo === "true",
              ...
              ? {
                  create: linkPreviewsToUse.map((preview: any) => {
                    ...
                    return {
                      ...
                      faviconUrl: preview.faviconUrl,
                      isVideo:
                        preview.isVideo === true || preview.isVideo === "true", // Ensure boolean true is preserved
                      embedUrl: preview.embedUrl ?? null,
                      author: preview.author ?? null,
                      platform: preview.platform ?? null,
                      site: preview.site ?? null,
    ...
    const searchableContent = [
      ...
      Array.isArray(post.subcategories)
        ? post.subcategories.map((subcat: any) => subcat.name).join(" ")
        : "",
      post.owner?.username,
    ]
      .filter(Boolean)
    ...
    console.log(
      "Search vector preview:",
      searchableContent.substring(0, 100) + "...",
    );
          ...
          if (mediaFile.storagePath) {
            ...
            if (oldPath.includes("draftmedia/")) {
              ...
              try {
                ...
                console.error(
                  "Error during file copy, falling back to download/upload:",
                  error,
                );
                await fallbackFileMigration(
                  mediaFile,
              ...
              if (mediaFile.thumbnailPath) {
                ...
                if (oldThumbnailPath.includes("draftmedia/")) {
                  const thumbnailNewPath = oldThumbnailPath.replace(
                    "draftmedia/",
                    "postmedia/",
                  );
              ...
              if (
                ...
                mediaFile.thumbnailPath.includes("draftmedia/")
              ) {
                // Update thumbnail path if needed
                newThumbnailPath = mediaFile.thumbnailPath.replace(
                  "draftmedia/",
                  "postmedia/",
                );
              }
              ...
              console.log("Media record created (file already in postmedia):", {
                filename: mediaFile.filename || mediaFile.id,
              });
            }
          }
    ...
    return NextResponse.json({
      ...
      message: isUpdate
        ...
        : "Draft converted to post successfully",
      isUpdate: isUpdate,
    });
  } catch (error) {
